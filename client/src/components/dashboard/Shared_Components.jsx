...
import { Video, Phone, Settings, Send, Mic, Download, FileText, 
    Save, MessageSquare, ScreenShare, Maximize2, X } from "lucide-react";
...
export const VideoCall = () => {
    ...
    const switchMediaStream = async (audioDeviceId, videoDeviceId) => {
      try {
        ...
        // Update tracks in the peer connection if it exists
    ...
    const startCall = async () => {
      setIsCallActive(true);
      if (!myStream) {
        const stream = await switchMediaStream(selectedAudioDeviceId, selectedVideoDeviceId);
    ...
    return (
      ...
      >
        ...
        <div className="flex-1 bg-gradient-to-b from-gray-900 to-indigo-900 flex flex-col items-center justify-center p-4">
          {isCallActive ? (
            <div className={`w-full h-full relative ${showChat ? 'flex' : ''}`}>
              <div className={`relative ${showChat ? 'w-3/4' : 'w-full'} h-full`}>
                ...
                <video
                  ...
                  muted
                  playsInline
                  className={`absolute bottom-3 right-3 w-36 h-24 bg-black rounded-lg border-2 border-white object-cover shadow-lg transition-opacity ${
                    videoEnabled ? "opacity-100" : "opacity-50"
                  }`}
                />
  
                <div className="absolute bottom-3 left-1/2 transform -translate-x-1/2 flex items-center gap-2 bg-black/30 backdrop-blur-sm p-1.5 rounded-full">
                  <button
                    onClick={endCall}
                    className="p-3 bg-red-500 hover:bg-red-600 text-white rounded-full shadow-md transition-colors"
                    title="End call"
                  >
                    <Phone size={18} />
                  </button>
                  <button
                    onClick={toggleMic}
                    className={`p-3 ${
                      micEnabled ? "bg-gray-700 hover:bg-gray-800" : "bg-yellow-600 hover:bg-yellow-700"
                  ...
                  <button
                    onClick={toggleVideo}
                    className={`p-3 ${
                      videoEnabled ? "bg-gray-700 hover:bg-gray-800" : "bg-yellow-600 hover:bg-yellow-700"
                    } text-white rounded-full shadow-md transition-colors`}
                  ...
                  <button
                    ...
                    className="p-3 bg-gray-700 hover:bg-gray-800 text-white rounded-full shadow-md transition-colors"
              ...
              {showChat && (
                <div className="w-1/4 h-full ml-4 bg-white rounded-lg shadow-lg flex flex-col">
            ...
            <div className="text-center py-8 px-4 max-w-md mx-auto">
              ...
              <p className="text-gray-300 mb-6 leading-relaxed">
                Start a video call to collaborate in real-time with your team members
              </p>
              <button
                onClick={startCall}
                className="px-6 py-3 bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white font-medium rounded-full flex items-center gap-2 mx-auto transition-all shadow-lg hover:shadow-indigo-500/30"
              >
                <Video size={18} />
              ...
              {callStatus === "error" && (
                <div className="mt-6 p-3 bg-red-500/20 border border-red-500/30 rounded-lg text-red-200 text-sm">
                  <p>Error connecting to call. Please check your camera and microphone permissions.</p>
                </div>
              )}
            </div>
          )}
